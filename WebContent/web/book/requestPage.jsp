			...
			<div class="contents">
				<div class="board_list_wrap">
					<table class="board_list table table-hover">
						<tbody>
							<c:forEach var="dto" begin="${ (curPage-1)*10 }" end="${ curPage*10-1 }" step="1" items="${ requestScope[requestScope.pageType] }">
								<tr class="row">
									<td class="col text-center"><img src="${ dto.coverLargeUrl }" alt="${ dto.title }" /></td>
				...
				<div class="text-center mb-3">
					<c:forEach var="i" begin="1" end="${ pageNum }" step="1">
						<c:choose>
							...
							<c:otherwise>
								<a href="book/genre.do?curPage=${ i }"><button type="button" class="btn btn-outline-secondary">${ i }</button></a>
							</c:otherwise>
						</c:choose>
					</c:forEach>
				</div>
